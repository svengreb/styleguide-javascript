...
To enable support for [React][npm-eslint-plugin-react] and [JSX A11Y][npm-eslint-plugin-jsx-a11y] plugin rules as well as compatibility integrations for other projects like [Prettier][], use the shareable rule configuration package [@arcticicestudio/eslint-config][npm-@arcticicestudio/eslint-config].
...
**This will not install the required [peer dependencies][node-b-peerdeps]** like the [eslint][npm-eslint] package itself. They must be installed separately as described below.
...
Therefore this package depends on the [`eslint-plugin-import`][npm-eslint-plugin-import] and [`eslint`][npm-eslint] package, defined as [peer dependencies][node-b-peerdeps], that must be installed separately either manually or using the [install-peerdeps][npm-install-peerdeps] helper package:
...
[npm-@arcticicestudio/eslint-config]: https://www.npmjs.com/package/@arcticicestudio/eslint-config
